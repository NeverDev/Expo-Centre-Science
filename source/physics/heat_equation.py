...
class HeatEquation(object):
    ...
    def __init__(self, temperature, dx, dy, density, thermal_conductivity, specific_heat_capacity, dt=None):
        ...
        if isinstance(specific_heat_capacity, np.ndarray) and specific_heat_capacity.shape == self.temperature.shape:
            ...
            raise Exception(
                "parameter specific_heat_capacity must be a numpy.ndarray of same size as temperature array")
        ...
        self.thermal_conductivity_dx_minus = np.minimum(self.thermal_conductivity[2:, 1:-1],
                                                        self.thermal_conductivity_middle)
        self.thermal_conductivity_dx_plus = np.minimum(self.thermal_conductivity[:-2, 1:-1],
                                                       self.thermal_conductivity_middle)
        self.thermal_conductivity_dy_minus = np.minimum(self.thermal_conductivity[1:-1, 2:],
                                                        self.thermal_conductivity_middle)
        self.thermal_conductivity_dy_plus = np.minimum(self.thermal_conductivity[1:-1, :-2],
                                                       self.thermal_conductivity_middle)
        ...
        if dt:
            ...
            if _dt > max_dt:
                raise Warning(
                    "Value of parameter dt {0} exceeds calculated stable maximum value {1}.".format(_dt, max_dt))
            self.dt = _dt
        else:
            self.dt = max_dt
    ...
    def boundary_temp(self):
        ...
        """
        stats = [(float(len(_)), sum(_)) for _ in
                 [self.temperature[0, :], self.temperature[-1, :], self.temperature[1:-1, 0],
